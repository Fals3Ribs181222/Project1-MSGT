...
async function loadAiMaterials() {
    const tbody = document.getElementById('aiMaterialsTableBody');
    const btnRefresh = document.getElementById('btnRefreshAiMaterials');
    if (!tbody) return;

    if (btnRefresh) { btnRefresh.disabled = true; btnRefresh.textContent = 'Refreshing...'; }
    window.tableLoading('aiMaterialsTableBody', 6, 'Loading…');

    const response = await window.api.get('files', { upload_type: 'ai' });
    ...
    tbody.innerHTML = files.length > 0
        ...
        : `<tr><td colspan="6" class="empty-state" style="padding:2.5rem;"><i class="ri-inbox-2-line" style="font-size:2rem;display:block;margin-bottom:0.5rem;opacity:0.35;"></i>No AI Training Resources uploaded yet.</td></tr>`;
...
export function init() {
    loadAiMaterials();
    const btnRefresh = document.getElementById('btnRefreshAiMaterials');
    if (btnRefresh) btnRefresh.addEventListener('click', loadAiMaterials);
}
