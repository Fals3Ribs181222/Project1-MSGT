...
function injectLeaderboardModal() {
    ...
    el.setAttribute('aria-labelledby', 'lbModalTitle');
    el.innerHTML = `
        <div class="class-modal" style="max-width:500px;">
            <div class="class-modal__header">
                <div>
                    <h3 id="lbModalTitle">Test Leaderboard</h3>
                    <p class="class-modal__meta" id="lbModalMeta"></p>
                </div>
                <button class="class-modal__close" id="lbModalClose" aria-label="Close">&times;</button>
            </div>
            <div class="class-modal__content">
                <div class="table-wrap">
                    <table aria-label="Test leaderboard">
                        <thead>
                            <tr>
                        ...
                        <tbody id="lbModalBody">
                            <tr><td colspan="4" class="loading-text">No marks recorded yet.</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
...
export async function init() {
    ...
    testId = window._pendingIdentifier;
    window._pendingIdentifier = null;

    if (!testId) {
        window.loadPage('page-test', 'list');
        return;
    }

    history.replaceState(null, '', `${location.search}#test#marks@${testId}`);
...
function renderPage() {
    ...
    const dateStr = currentTest.date ? new Date(currentTest.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : '—';
    const schools = Array.isArray(currentTest.schools) ? currentTest.schools : [];
    document.getElementById('testMeta').innerHTML = [
        currentTest.subject && `<span class="meta-chip">${window.esc(currentTest.subject)}</span>`,
        currentTest.grade && `<span class="meta-chip">Grade ${window.esc(String(currentTest.grade))}</span>`,
        `<span class="meta-chip"><i class="ri-calendar-line" aria-hidden="true"></i> ${dateStr}</span>`,
        `<span class="meta-chip"><i class="ri-award-line" aria-hidden="true"></i> ${currentTest.max_marks} marks</span>`,
        schools.length > 0 && `<span class="meta-chip"><i class="ri-building-line" aria-hidden="true"></i> ${schools.map(s => window.esc(s)).join(', ')}</span>`,
    ].filter(Boolean).join('');
    ...
    const marksMap = {};
    existingMarks.forEach(m => { marksMap[m.student_id] = { id: m.id, marks: m.marks_obtained }; });
...
function updateSendBtns() {
    ...
    if (btnMarks) {
        btnMarks.innerHTML = `<i class="ri-whatsapp-line" aria-hidden="true"></i> Send Marks (${marksCount})`;
    ...
    if (btnMissed) {
        btnMissed.innerHTML = `<i class="ri-whatsapp-line" aria-hidden="true"></i> Send Not Attempted (${missedCount})`;
...
function openLeaderboardModal() {
    ...
    document.getElementById('lbModalTitle').textContent = `${currentTest.title} — Leaderboard`;
    document.getElementById('lbModalMeta').textContent = `${currentTest.subject} · Grade ${currentTest.grade} · Max: ${max}`;
    ...
    const withoutMarks = eligibleStudents.filter(s => savedMap[s.id] === undefined || savedMap[s.id] === '');
...
function wireEvents() {
    document.getElementById('marksForm').addEventListener('submit', async (e) => {
        ...
        try {
            const ops = [];
            if (idsToDelete.length > 0) ops.push(...idsToDelete.map(id => api.delete('marks', id)));
            if (marksPayload.length > 0) ops.push(api.upsert('marks', marksPayload));
            ...
            if (!failed) {
                ...
                throw new Error(failed.error || 'Failed to save marks.');
            }
        } catch (err) {
    ...
    document.getElementById('btnSendMarks')?.addEventListener('click', async () => {
        ...
        try {
            ...
            for (let i = 0; i < marksData.length; i++) {
                ...
                btn.innerHTML = `<i class="ri-whatsapp-line" aria-hidden="true"></i> Sending ${i + 1}/${marksData.length}...`;
    ...
    document.getElementById('btnSendNotAttempted')?.addEventListener('click', async () => {
        ...
        try {
            ...
            for (let i = 0; i < absentIds.length; i++) {
                ...
                btn.innerHTML = `<i class="ri-whatsapp-line" aria-hidden="true"></i> Sending ${i + 1}/${absentIds.length}...`;
    ...
    document.addEventListener('input', (e) => {
        if (e.target.classList.contains('student-mark-input')) updatePerformanceSummary();
    });
}
