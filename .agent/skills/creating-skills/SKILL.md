...
description: Generates high-quality, predictable, and efficient skill directories based on user requirements. Use when the user asks to create a new skill or wants to build a new agent capability.
---
...
- User wants to build a new agent capability or workflow
- User mentions "Skill Creator"

## Workflow
1. Analyze the user's request for the new skill.
2. Determine a gerund-based name (e.g., `testing-code`) and a concise 3rd-person description.
3. Establish the `.agent/skills/<skill-name>/` directory structure.
4. Draft the `SKILL.md` file using the required YAML frontmatter and specific sections.
5. Create optional `scripts/`, `examples/`, or `resources/` directories if needed.
6. Validate that the generated logic adheres to the writing principles (conciseness, progressive disclosure, degrees of freedom).
