...
async function loadTestimonialComponent() {
    try {
        const response = await fetch('components/modals/add_testimonial');
        if (response.ok) {
...
export function activateTab(tabSlug) {
    const _map = { 'testimonials': 'pillViewTestimonials', 'add_testimonial': 'pillAddTestimonial' };
    const pillId = _map[tabSlug] || _map['testimonials'];
